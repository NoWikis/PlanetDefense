...
public class PlayerControllerTest : MonoBehaviour {
	...
	void movePlayer(InputDevice inputDevice) {
		Vector3 examplePos = new Vector3 (-inputDevice.LeftStickX, -inputDevice.LeftStickY, 0);
		
		if (examplePos.magnitude < .2)
			return;
		
		float angler = Util.getAngleVector (examplePos, Vector2.zero);
		transform.localPosition =  Quaternion.Euler (0, 0, angler) * new Vector3(1, 0, 0);
		transform.localRotation =  Quaternion.Euler (0, 0, angler);
		Debug.Log (inputDevice.LeftStickX + "  " + inputDevice.LeftStickY);
	}
